...
class DisplayStateService:
    async def get_display_state(self, db: AsyncSession, display_id: uuid.UUID) -> dict | None:
        logger.debug("Сбор состояния дисплея: display_id=%s", display_id)
        display = await db.get(Display, display_id)
        if display is None:
            logger.warning("Дисплей не найден при сборе состояния: display_id=%s", display_id)
            return None

        template = await self._get_template_state(db, display_id)
        emergency = await self._get_active_emergency_state(db, display_id)

        logger.debug(
            "Состояние дисплея собрано: display_id=%s has_template=%s has_emergency=%s",
            display_id,
        ...
        return {
            "type": "display_state",
            "display": self._serialize_display(display),
            "template": template,
            "emergency": emergency,
        }

    async def _get_template_state(self, db: AsyncSession, display_id: uuid.UUID) -> dict | None:
        ...
        if assignment is None:
            logger.debug("Шаблон не назначен: display_id=%s", display_id)
            return None

        template = await db.get(Template, assignment.template_id)
        if template is None:
            logger.warning(
                "Назначение шаблона есть, но шаблон не найден: display_id=%s template_id=%s",
                display_id,
        ...
        widgets = [
            self._serialize_widget(widget, static_content)
    ...
    async def _get_active_emergency_state(self, db: AsyncSession, display_id: uuid.UUID) -> dict | None:
        ...
        if emergency is None:
            logger.debug("Активная ЧС не найдена: display_id=%s", display_id)
            return None

        logger.debug(
            "Активная ЧС найдена: display_id=%s emergency_id=%s",
            display_id,
            emergency.id,
        )
        return {
            ...
            "starts_at": self._to_iso(emergency.starts_at),
            "ends_at": self._to_iso(emergency.ends_at),
            "created_at": self._to_iso(emergency.created_at),
            "reset_at": self._to_iso(emergency.reset_at),
        }


    def _serialize_display(self, display: Display) -> dict:
        return {
            "id": str(display.id),
            "title": display.title,
            ...
            "last_seen_at": self._to_iso(display.last_seen_at),
        }


    def _serialize_widget(self, widget: TemplateWidget, static_content: StaticContent | None) -> dict:
        return {
            "id": str(widget.id),
            "type": widget.type,
            ...
            "static_content": self._serialize_static_content(static_content),
        }


    def _serialize_static_content(self, static_content: StaticContent | None) -> dict | None:
        ...
        return {
            ...
            "valid_from": self._to_iso(static_content.valid_from),
            "valid_to": self._to_iso(static_content.valid_to),
        }
